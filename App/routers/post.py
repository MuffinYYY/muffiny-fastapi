...
async def upload(file: UploadFile, Authorize: AuthJWT = Depends(), db: Session = Depends(get_db)):
    ...
    save_path = 'C:\\Users\\muffi\\Desktop\\Python projects\\frontend\\public\\images'  #For desktop
    #save_path = 'C:\\Users\\muffi\\github reps\\muffiny-fastapi\\frontend\\public\\images' #For laptop
...
def get_all_amogus(db: Session = Depends(get_db), limit: int = 9999, skip: int = 0, search : Optional[str] = ""): #Limit is query parameter, so we can limit how many rows we get in our response, by default it's 10
